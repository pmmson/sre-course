...
2. Разобраться в ограничении кол-ва коннектов на HAProxy в районе 200. Это может вызывать нестабильную работу приложения, появление лавинообразного роста ошибок и рестарт подов.
3. Для стабильной работы приложения пересмотреть архитектуру - для системы мониторинга ввести свой сервер, по потреблению памяти перераспределить ее между вирутальными машинами в зависимости от потребления, повторить проведение тестов.
4. Приложение начинает деградировать при росте кол-ва прогнозов больше, чем один проноз на город. Необходимо пересмотреть подход к выборке данных из БД в сторону батч запросов, введение дополнительного метода получения прогноза по городу, введение индексов в таблицу, рассмотреть возможность секционирования по дате прогноза...
